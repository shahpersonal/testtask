...
class Database {
    ...
    function Handle_Mysql_Error($sql, $err, $errno) {
        ...
        error_log(SITETITLE . '/', $queryerror, DEST_EMAIL, ADMIN_EMAIL);

        echo __FILE__;
        echo '<br />';

        echo $queryerror;exit;

        echo basename($_SERVER['PHP_SELF']);
        echo '<br />';
        die("This functionlity under development");
        $msg = $_SERVER['REQUEST_URI'] . '<br />' . SITETITLE . '< br />' . $queryerrorL . "<br />" . $queryerror;
        
        $message = $_SERVER['REQUEST_URI'] . '<br />' . SITETITLE . '< br />' . $queryerror;
        $subject = 'Website Support Service :' . $_SERVER['SERVER_NAME'];
        $headers = 'MIME-Version: 1.0' . "\r\n";
        $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
        $headers .= 'From: Site Admin <webmaster@streetsmartuniversity.co>' . "\r\n";   // http://www.streetsmartuniversity.co/
        mail(DEVEMAILID, $subject, $message, $headers);
    ...
    function SelectDataJoin($fields, $tables, $join, $order = array(), $group = array(), $limit = "", $offset = 0, $print_flag = 0) {
        $fields = implode(",", $fields);
        $tables = implode(",", $tables);
        if (!empty($join)) {
            $join = $join;
